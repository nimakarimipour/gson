...
public class JsonReader implements Closeable {
   ...
   */
  @SuppressWarnings("NullAway") public String nextString() throws IOException {
    int p = peeked;
    if (p == PEEKED_NONE) {
      p = doPeek();
    }
   ...
   * consuming it. If the next token is a string, this method will attempt to parse it as a double
   ...
   * consuming it. If the next token is a string, this method will attempt to parse it as a long. If
   * the next token's numeric value cannot be exactly represented by a Java {@code long}, this
   * method throws.
   *
   * @throws IllegalStateException if the next token is not a literal value.
   ...
   * consuming it. If the next token is a string, this method will attempt to parse it as an int. If
   * the next token's numeric value cannot be exactly represented by a Java {@code int}, this method
   * throws.
   *
   * @throws IllegalStateException if the next token is not a literal value.
   * @throws NumberFormatException if the next literal value cannot be parsed as a number, or
