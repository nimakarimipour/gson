    ...
    void add(Node<K, V> node) {
      ...
      for (int scale = 4; (size & scale - 1) == scale - 1; scale *= 2) {
        if (leavesSkipped == 0) {
          ...
          Node<K, V> center = right != null ? right.parent : null;
          Node<K, V> left = center != null ? center.parent : null;
          if (left != null) { // Check if left is non-null
          ...
          if (center != null) { // Check if center is non-null
            center.right = right;
            center.height = right.height + 1;
            right.parent = center;
          }
